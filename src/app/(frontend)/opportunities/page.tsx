...
export default async function OpportunitiesPage({ searchParams: searchParamsPromise }: Args) {
  ...
  const countries = SADC_COUNTRIES
  const activeCountryCount = new Set(allOpportunities.docs.map((opportunity) => opportunity.country).filter(Boolean)).size
  ...
  if (country && SADC_COUNTRIES.includes(country as (typeof SADC_COUNTRIES)[number])) {
  ...
  return (
    <>
      <PageHeader
        backgroundImage="/home/gallery-douala.jpg"
        backgroundPosition="center 48%"
        title="Opportunities"
        lede={
          'Illustrative opportunities across the network. Confirm availability and referral terms with the team before making an introduction.'
        }
        meta={
          PUBLISHED_OPPORTUNITIES_AVAILABLE && allOpportunities.totalDocs > 0 ? (
            <p>
              ...
              <span className="tabular text-white">{activeCountryCount}</span>{' '}
              {activeCountryCount === 1 ? 'country' : 'countries'}
            </p>
          ) : null
        }
      />
